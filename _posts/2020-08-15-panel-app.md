...
## Dashboard with Panel using Datashader and Altair

Panel is a solution for dashboarding in Python. Supports live code in a Jupyter notebook and deploying the same code to a server. Also provides support for Datashader with visualizing large datasets.

In this example app the data coming from the parking meter transactions in Paris. The main part is a heatmap of the transactions using Datashader. The other chart shows the monthly mean income from the parking meters in the time window given. As the user zooms in the map, the second chart updates to show only the data selected.
```python
import numpy as np
import pandas as pd
...
2. Enables the notebook to be served from `localhost`. We can execute `panel serve --show app.ipynb` from the command line and the app running live at `http://localhost:5006/app`
